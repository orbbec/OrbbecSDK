...
void get_property_list(ob_device *device, ob_property_item **property_list,
                       int *size);
void printf_property_list(ob_device *device);
void set_property_value(ob_device *device, ob_property_item property_item,
                        const char *value);
      ...
      if (strcmp(str, "?") != 0) {
        ...
        if (p) {
          if (strcmp(p, "get") != 0 && strcmp(p, "set") != 0) {
            printf(
                "Property control usage: [property index] [set] [property "
                "value] or [property index] [get]\n");
            continue;
          }
          strcpy(command, p);
        } else {
          printf(
              "Property control usage: [property index] [set] [property value] "
              "or [property index] [get]\n");
          continue;
        }
        ...
        int size = 0;
        ob_property_item *property_list =
            (ob_property_item *)malloc(sizeof(ob_property_item) * 100);
        ...
        printf(
            "Please select property.(Property control usage: [property number] "
...
ob_device *select_device(ob_device_list *device_list) {
  ...
  for (int i = 0; i < dev_count; i++) {
    ...
    check_error(g_error);
    const char *sn =
        ob_device_list_get_device_serial_number(device_list, i, &g_error);
    check_error(g_error);
  ...
  ob_device *device =
      ob_device_list_get_device(device_list, dev_index, &g_error);
  check_error(g_error);
...
void printf_property_list(ob_device *device) {
  ...
  int i = 0, index = 0;
  for (i = 0; i < size; i++) {
    ob_property_item property_item =
        ob_device_get_supported_property(device, i, &g_error);
    check_error(g_error);
    if (property_item.type != OB_STRUCT_PROPERTY &&
        (property_item.permission & OB_PERMISSION_READ)) {
      char str[100] = "";
      ...
      switch (property_item.type) {
        ...
        case OB_INT_PROPERTY:
          int_range = ob_device_get_int_property_range(device, property_item.id,
                                                       &g_error);
          check_control_property_error(g_error);
          sprintf(str, "Int value(min:%d, max:%d, step:%d,)", int_range.min,
                  int_range.max, int_range.step);
          break;
        case OB_FLOAT_PROPERTY:
          float_range = ob_device_get_float_property_range(
              device, property_item.id, &g_error);
          check_control_property_error(g_error);
          sprintf(str, "Float value(min:%f, max:%f, step:%f,)", float_range.min,
                  float_range.max, float_range.step);
          break;
        default:
          break;
      }

      printf("%d: %s, permission=%s, range=%s\n", index, property_item.name,
             permissionTypeToString(property_item.permission), str);
      index++;
    }
  }
  printf(
...
void get_property_list(ob_device *device, ob_property_item **property_list,
  ...
  uint32_t propertySize =
      ob_device_get_supported_property_count(device, &g_error);
  check_error(g_error);
  int i = 0;
  int index = 0;
...
void set_property_value(ob_device *device, ob_property_item property_item,
  ...
  switch (property_item.type) {
    case OB_BOOL_PROPERTY:
      ...
      ob_device_set_bool_property(device, property_item.id, bool_value,
                                  &g_error);
      check_control_property_error(g_error);
    ...
    case OB_FLOAT_PROPERTY:
      ...
      ob_device_set_float_property(device, property_item.id, float_value,
                                   &g_error);
      check_control_property_error(g_error);
...
void get_property_value(ob_device *device, ob_property_item property_item) {
  ...
  switch (property_item.type) {
    case OB_BOOL_PROPERTY:
      bool_ret =
          ob_device_get_bool_property(device, property_item.id, &g_error);
    ...
    case OB_FLOAT_PROPERTY:
      float_ret =
          ob_device_get_float_property(device, property_item.id, &g_error);
      check_control_property_error();
