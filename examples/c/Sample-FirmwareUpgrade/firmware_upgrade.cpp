...
static void device_changed_callback(ob_device_list *removed,
                                    ob_device_list *added, void *user_data);
static void device_upgrade_callback(ob_upgrade_state state, const char *message,
...
int main(int argc, char **argv) {
  ...
  ob_set_device_changed_callback(ctx, device_changed_callback,
                                 &callback_user_data_, &error);
  check_error(error);
  ...
  // wait reboot complete
  {
    ...
    wait_reboot_condition_.wait_for(lk, std::chrono::milliseconds(60000),
  ...
  while (true) {
    if (kbhit()) {
      // Get the value of the pressed key, if it is the esc key, exit the
      // program
      int key = getch();
      if (key == ESC_KEY) {
        break;
...
bool upgrade_firmware(ob_device *device, const char *firmwarePath) {
  ...
  ob_error *error = NULL;
  ob_device_upgrade(device, firmwarePath, device_upgrade_callback, false,
                    &callback_user_data_, &error);
  check_error(error);
...
void device_upgrade_callback(ob_upgrade_state state, const char *message,
                             uint8_t percent, void *user_data) {
  if (state == STAT_START) {
...
void device_changed_callback(ob_device_list *removed, ob_device_list *added,
                             void *user_data) {
  // Wait device attache and detach event after device upgrade firmware
  // completed and call reboot.
  ob_error *error = NULL;
