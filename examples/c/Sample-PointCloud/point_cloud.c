...
ob_pipeline *pipeline = NULL;  // pipeline, used to open the Color and Depth
                               // streams after connecting the device
...
int main(int argc, char **argv) {
  ...
  // Create a pipeline to open the Color and Depth streams after connecting the
  // device
  pipeline = ob_create_pipeline(&error);
  check_error(error);

  // Create config to configure the resolution, frame rate, and format of Color
  // and Depth streams
  ...
  ob_stream_profile_list *profiles =
      ob_pipeline_get_stream_profile_list(pipeline, OB_SENSOR_DEPTH, &error);
  check_error(error);
  // Open the default profile of Depth Sensor, which can be configured through
  ...
  profiles =
      ob_pipeline_get_stream_profile_list(pipeline, OB_SENSOR_COLOR, &error);
  if (error) {
    printf("Current device is not support color sensor!\n");
    // If there is no Color Sensor point cloud conversion resolution configured
    // as depth resolution
    ob_config_set_d2c_target_resolution(
        config, ob_video_stream_profile_width(depth_profile, &error),
        ob_video_stream_profile_height(depth_profile, &error), &error);
    ob_config_set_depth_scale_require(config, false, &error);
    ob_delete_error(error);
    error = NULL;
  }

  // Open the default profile of Color Sensor, which can be configured through
  // the configuration file
  if (profiles) {
  ...
  // Create a point cloud Filter object (device parameters will be obtained
  ...
  // Obtain the current open-stream camera parameters from the pipeline and pass
  // them to the point cloud filter
    ...
    if (kbhit()) {
      ...
      if (key == 'R' || key == 'r') {
        ...
        while (count++ < 10) {
          ...
          ob_frame *frameset =
              ob_pipeline_wait_for_frameset(pipeline, 100, &error);
          check_error(error);
          if (frameset != NULL) {
            ob_pointcloud_filter_set_point_format(point_cloud,
                                                  OB_FORMAT_RGB_POINT, &error);
            check_error(error);
            ...
            ob_delete_frame(frameset,
                            &error);  // Destroy frameSet to reclaim memory
            check_error(error);
            if (points_created) {
              break;
            }
        ...
        while (count++ < 10) {
          ...
          check_error(error);
          if (frameset != NULL) {
            ob_pointcloud_filter_set_point_format(point_cloud, OB_FORMAT_POINT,
                                                  &error);
            check_error(error);
            ob_frame *pointsFrame =
                ob_filter_process(point_cloud, frameset, &error);
