...
int main(int argc, char **argv) {
  ...
  printf("SDK version: %d.%d.%d\n", ob_get_major_version(),
         ob_get_minor_version(), ob_get_patch_version());
  ...
  // Check whether the camera depth working mode is supported, currently
  // (December 5, 2022) only the Gemini2 binocular camera supports the depth
  // working mode
  if (!ob_device_is_property_supported(dev, OB_STRUCT_CURRENT_DEPTH_ALG_MODE,
  ...
  ob_depth_work_mode cur_work_mode =
      ob_device_get_current_depth_work_mode(dev, &error);
  check_error(error);
  printf("Current depth work mode: %s\n", cur_work_mode.name);

  // Query the list of depth working modes supported by the camera
  ob_depth_work_mode_list *mode_list =
      ob_device_get_depth_work_mode_list(dev, &error);
  check_error(error);
  ...
  for (uint32_t i = 0; i < mode_count; i++) {
    ob_depth_work_mode mode =
        ob_depth_work_mode_list_get_item(mode_list, i, &error);
    check_error(error);
  ...
  if (mode_index >= 0 && mode_index < mode_count) {
    ob_depth_work_mode mode =
        ob_depth_work_mode_list_get_item(mode_list, mode_index, &error);
    check_error(error);
  ...
  // This is the end of switching the camera depth mode, you can use the
  // pipeline to open the camera to get the stream Notice:
  // 1. If you need to switch the camera depth mode, you must open the data
  // stream after switching the depth working mode; the effective resolution
  // supported by each camera depth mode is different
  // 2. If the data stream has been opened using the pipeline, the original
  // pipeline must be released before switching camera depth mode.  otherwise,
  // wild pointer or memory leak will be caused.
