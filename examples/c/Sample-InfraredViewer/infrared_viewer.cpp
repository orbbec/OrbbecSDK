...
ob_device *device = nullptr;  // device, obtained through the pipeline, and the
                              // Infrared image can be set through the device
...
void frameset_proc_func() {
  while (!thread_exit) {
    ...
    if (!thread_exit && frameset_ready) {
      ob_frame *ir_frame = ob_frameset_ir_frame(frameset_for_render, &error);
      check_error(error);
      if (ir_frame != nullptr) {
        std::vector<ob_frame *> frames = {ir_frame};
...
int main(int argc, char **args) {
  ...
  if (ob_device_is_property_supported(device,
    ...
    ob_device_set_int_property(device, OB_PROP_IR_CHANNEL_DATA_SOURCE_INT,
                               dataSource, &error);
    check_error(error);
  }
  check_error(error);
  ...
  if (ob_device_is_property_supported(device, OB_PROP_IR_MIRROR_BOOL,
                                      OB_PERMISSION_WRITE, &error)) {
    // Set mirroring
    ob_device_set_bool_property(device, OB_PROP_IR_MIRROR_BOOL, true, &error);
    check_error(error);
  }
  check_error(error);
  ...
  win = new Window("InfraredViewer",
                   ob_video_stream_profile_width(ir_profile, &error),
                   ob_video_stream_profile_height(ir_profile, &error));
  check_error(error);
  ...
  // Stop the frameset capture and processing thread
  thread_exit = true;
  frameset_proc_cv.notify_all();
