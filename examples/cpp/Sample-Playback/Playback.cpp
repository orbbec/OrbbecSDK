...
int main(int argc, char **argv) try {
  ...
  std::cout << "======================DeviceInfo: name : " << deviceInfo->name()
  ...
  std::cout << "======================Camera params : rgb width:"
            << cameraParam.rgbIntrinsic.width
            << " rgb height: " << cameraParam.rgbIntrinsic.height
            << " depth width: " << cameraParam.depthIntrinsic.width
            << " depth height: " << cameraParam.rgbIntrinsic.height
            << std::endl;

  Window app("Playback", 1280, 480);

  // frame data callback
  ob::PlaybackCallback callback =
      ([&](std::shared_ptr<ob::Frame> frame, void *pCookies) {
