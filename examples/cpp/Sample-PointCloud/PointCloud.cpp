...
void saveRGBPointsToPly(std::shared_ptr<ob::Frame> frame,
                        std::string fileName) {
...
int main(int argc, char **argv) try {
  ...
  try {
    ...
    config->setD2CTargetResolution(depthProfile->width(),
                                   depthProfile->height());
  ...
  // operation prompt
  std::cout << "Press R or r to create RGBD PointCloud and save to ply file! "
            << std::endl;
  std::cout << "Press D or d to create Depth PointCloud and save to ply file! "
            << std::endl;
