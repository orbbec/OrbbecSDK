...
int main(int argc, char **argv) try {
  ...
  while (app) {
    // Render a set of frame in the window, here will render the depth, color or
    // infrared frames of all devices, RENDER_GRID means that all frames will be
    // rendered in a grid arrangement
    frames.clear();
    {
...
void StartStream(std::vector<std::shared_ptr<ob::Pipeline>> pipes) {
  ...
  for (auto &&pipe : pipes) {
    ...
    if (depthProfileList) {
      // Open the default profile of Depth Sensor, which can be configured
      // through the configuration file
      depthProfile = std::const_pointer_cast<ob::StreamProfile>(
                         depthProfileList->getProfile(0))
    ...
    try {
      ...
      if (colorProfileList) {
        // Open the default profile of Color Sensor, which can be configured
        // through the configuration file
        colorProfile = std::const_pointer_cast<ob::StreamProfile>(
                           colorProfileList->getProfile(0))
                           ->as<ob::VideoStreamProfile>();
      }
