...
static std::ostream &operator<<(std::ostream &os,
                                const OBDepthWorkMode &dataType);
...
int main(int argc, char *argv[]) try {
  ...
  // working mode
  if (!device->isPropertySupported(OB_STRUCT_CURRENT_DEPTH_ALG_MODE,
  ...
  if (depthModeList->count() > 0) {
    ...
    if (index >= 0 && index < depthModeList->count()) {  // legitimacy check
      ...
      if (strcmp(curDepthMode.name, (*depthModeList)[index].name) == 0) {
        std::cout << "Switch depth work mode success! currentDepthMode: "
                  << curDepthMode << std::endl;
      } else {
        std::cout << "Switch depth work mode failed!" << std::endl;
      }
    } else {
      std::cout << "switchDepthMode faild. invalid index: " << index
                << std::endl;
    }
  }
...
static std::ostream &operator<<(std::ostream &os,
                                const OBDepthWorkMode &workMode) {
  os << "{name: " << workMode.name << ", checksum: "
     << toHexText(workMode.checksum, sizeof(workMode.checksum)) << "}";
  return os;
}