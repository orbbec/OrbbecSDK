...
/**
 * @brief Enumeration value describing all attribute control commands of the
 * device
 */
typedef enum {
  /**
  ...
  /**
   * @brief the depth precision level, which may change the depth frame data
   * unit, needs to be confirmed through the ValueScale interface of DepthFrame
   */
  ...
  /**
   * @brief laser mode, the firmware terminal currently only return 1: IR Drive,
   * 2: Torch
   */
  OB_PROP_LASER_MODE_INT = 79,

  /**
   * @brief brt2r-rectify function switch (brt2r is a special module on mx6600),
   * 0: Disable, 1: Rectify Enable
   */
  ...
  /**
   * @brief Disparity to depth switch, 0: off, the depth stream outputs the
   * disparity map; 1. On, the depth stream outputs the depth map.
   */
  OB_PROP_DISPARITY_TO_DEPTH_BOOL = 85,

  /**
   * @brief BRT function switch (anti-background interference), 0: Disable, 1:
   * Enable
   */
  OB_PROP_BRT_BOOL = 86,
  ...
  /**
   * @brief External signal trigger restart function switch, 0: Disable, 1:
   * Enable
   */
  ...
  /**
   * @brief Custom RGB cropping switch, 0 is off, 1 is on custom cropping, and
   * the ROI cropping area is issued
   */
  ...
  /**
   * @brief Enable send reset device time signal to other device. true: enable,
   * false: disable
   */
  ...
  /**
   * @brief Number frame to capture once a 'OB_PROP_CAPTURE_IMAGE_SIGNAL_BOOL'
   * effect. range: [1, 255]
   */
  OB_PROP_CAPTURE_IMAGE_FRAME_NUMBER_INT = 113,

  /**
   * @brief Right IR sensor flip state. true: flip image, false: origin,
   * default: false
   */
  ...
  /**
   * @brief Get hardware laser energy level which real state of laser element.
   * OB_PROP_LASER_ENERGY_LEVEL_INT（99）will effect this command which it
   * setting and changed the hardware laser energy level.
   */
  ...
  /**
   * @brief Device development mode switch, optional modes can refer to the
   * definition in @ref OBDeviceDevelopmentMode,the default mode is
   * @ref OB_USER_MODE
  ...
  /**
   * @brief Multi-DeviceSync synchronized signal trigger out is enable state.
   ...
   * @attention This command can only be written, and the parameter value must
   * be true. The command takes effect after restarting the device.
   */
  OB_PROP_RESTORE_FACTORY_SETTINGS_BOOL = 131,

  /**
   * @brief Enter recovery mode (flashing mode) when boot the device
   * @attention The device will take effect after rebooting with the enable
   * option. After entering recovery mode, you can upgrade the device system.
   * Upgrading the system may cause system damage, please use it with caution.
   */
  OB_PROP_BOOT_INTO_RECOVERY_MODE_BOOL = 132,

  /**
   * @brief Query whether the current device is running in recovery mode
   * (read-only)
   */
  ...
  /**
   * @brief A list of depth accuracy levels, returning an array of uin16_t,
   ...
   * @brief Using for get last static ip config，witch is record in device flash
   * when user set static ip config
   *
   * @attention read only
   */
  ...
  /**
   * @brief Automatic exposure of depth camera (infrared camera will be set
   * synchronously under some models of devices)
   */
  OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL = 2016,

  /**
   * @brief Depth camera exposure adjustment (infrared cameras will be set
   * synchronously under some models of devices)
   */
  OB_PROP_DEPTH_EXPOSURE_INT = 2017,

  /**
   * @brief Depth camera gain adjustment (infrared cameras will be set
   * synchronously under some models of devices)
   */
  OB_PROP_DEPTH_GAIN_INT = 2018,

  /**
   * @brief Infrared camera auto exposure (depth camera will be set
   * synchronously under some models of devices)
   */
  OB_PROP_IR_AUTO_EXPOSURE_BOOL = 2025,

  /**
   * @brief Infrared camera exposure adjustment (some models of devices will set
   * the depth camera synchronously)
   */
  OB_PROP_IR_EXPOSURE_INT = 2026,

  /**
   * @brief Infrared camera gain adjustment (the depth camera will be set
   * synchronously under some models of devices)
   */
  OB_PROP_IR_GAIN_INT = 2027,

  /**
   * @brief Select Infrared camera data source channel. If not support throw
   * exception. 0 : IR stream from IR Left sensor; 1 : IR stream from IR Right
   * sensor;
   */
  OB_PROP_IR_CHANNEL_DATA_SOURCE_INT = 2028,

  /**
   * @brief Depth effect dedistortion, true: on, false: off. mutually exclusive
   * with D2C function, RM_Filter disable When hardware or software D2C is
   * enabled.
   */
  ...
  /**
   * @brief The enable/disable switch for IR short exposure function, supported
   * only by a few devices.
   */
  ...
  /**
   * @brief Depth data unpacking function switch (each open stream will be
   * turned on by default, support RLE/Y10/Y11/Y12/Y14 format)
   */
  OB_PROP_SDK_DEPTH_FRAME_UNPACK_BOOL = 3007,

  /**
   * @brief IR data unpacking function switch (each current will be turned on by
  ...
  /**
   * @brief Left IR frame data unpacking function switch (each current will be
   * turned on by default, support RLE/Y10/Y11/Y12/Y14 format)
   */
  OB_PROP_SDK_IR_LEFT_FRAME_UNPACK_BOOL = 3011,

  /**
   * @brief Right IR frame data unpacking function switch (each current will be
   * turned on by default, support RLE/Y10/Y11/Y12/Y14 format)
   */
...
#define OB_PROP_TIMER_RESET_TRIGGLE_OUT_ENABLE_BOOL \
